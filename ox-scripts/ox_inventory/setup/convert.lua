...
local function ConvertQB()
	...
	if plates then
		...
		local trunk = MySQL.query.await('SELECT identifier, items FROM inventories')
		...
		if trunk then
			...
			for _, v in pairs(trunk) do
				local plate = SplitStr(v.identifier, "-")[2]
				local owner = plates[plate]
				...
				if owner then
					...
					if not vehicles[owner][plate] then
						...
						vehicles[owner][plate] = true
						count = count + 1
						parameters[count] = { 'UPDATE player_vehicles SET trunk = ? WHERE plate = ? AND citizenid = ?', { json.encode(inventory), plate, owner } }
					end
				end
			end
		...
		local glovebox = MySQL.query.await('SELECT identifier, items FROM inventories')
			...
			for _, v in pairs(glovebox) do
				...
				if owner then
					...
					if not vehicles[owner][plate] then
						...
						vehicles[owner][plate] = true
						count = count + 1
						parameters[count] = { 'UPDATE player_vehicles SET glovebox = ? WHERE plate = ? AND citizenid = ?', { json.encode(inventory), plate, owner } }
					end
				end
			end
	...
	local qbEvidence = MySQL.query.await("SELECT items FROM inventories WHERE items LIKE '% | Drawer%'")
