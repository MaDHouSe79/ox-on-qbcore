...
if shared.target then
	...
	if shared.useParkingFee then
		parkingTarget[#parkingTarget + 1] = {type = "client", icon = "fas fa-parking", label = locale('pay_park_fee'), onSelect = function(data) return Inventory.PayFee(data.entity) end, distance = 2}
	end

	if shared.useLockpickParking then
		parkingTarget[#parkingTarget + 1] = {type = "client", icon = "fas fa-parking", label = locale("lockpick"), canInteract = Inventory.HasLockpick, onSelect = function(data) return Inventory.LockpickParkmeter(data.entity) end, distance = 2}
	end
	exports.ox_target:addModel(Inventory.Parkmeters, parkingTarget)


	if shared.useLockpickCellphone then
		exports.ox_target:addModel(Inventory.Cellphones, {icon = 'fas fa-phone', label = locale('lockpick'), canInteract = Inventory.HasLockpick, onSelect = function(data) return Inventory.LockpickCellphone(data.entity) end, distance = 2})
	end

	exports.ox_target:addModel(Inventory.Binbags, {icon = 'fa-trash', label = locale('search_binbag'), onSelect = function(data) return Inventory.OpenBinbag(data.entity) end, distance = 2})
    ...
    exports.ox_target:addGlobalVehicle({icon = 'fas fa-truck-ramp-box', label = locale('open_label', locale('storage')), distance = 1.5, canInteract = Inventory.CanAccessTrunk, onSelect = function(data) return Inventory.OpenTrunk(data.entity) end})

else
	local binbags = table.create(0, #Inventory.Binbags)
	for i = 1, #Inventory.Binbags do binbags[Inventory.Binbags[i]] = true end
	Inventory.Binbags = binbags

	local dumpsters = table.create(0, #Inventory.Dumpsters)
	for i = 1, #Inventory.Dumpsters do dumpsters[Inventory.Dumpsters[i]] = true end
	Inventory.Dumpsters = dumpsters

	if shared.useLockpickParking then
		local parkmeters = table.create(0, #Inventory.Parkmeters)
		for i = 1, #Inventory.Parkmeters do parkmeters[Inventory.Parkmeters[i]] = true end
		Inventory.Parkmeters = parkmeters
	end

	if shared.useLockpickCellphone then
		local cellphones = table.create(0, #Inventory.Cellphones)
		for i = 1, #Inventory.Cellphones do cellphones[Inventory.Cellphones[i]] = true end
