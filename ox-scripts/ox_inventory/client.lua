...
lib.callback.register('ox_inventory:getVehicleData', function(netid)
	...
	if entity then return GetEntityModel(entity), GetVehicleClass(entity) end
end)

RegisterNetEvent("ox_inventory:deleteped", function(netid)
	local entity = NetToPed(netid)
    local closedPed = lib.getClosestPed(GetEntityCoords(PlayerPedId()))
