...
function fuel.startFueling(vehicle, isPump)
	...
	local durability = 0
	if 100 - fuelAmount < config.refillValue then return lib.notify({ type = 'error', description = locale('tank_full') }) end
	if isPump then
		price = 0
		moneyAmount = utils.getMoney()
		if config.priceTick > moneyAmount then return lib.notify({type = 'error', description = locale('not_enough_money', config.priceTick)}) end
	...
	elseif state.petrolCan.metadata.ammo <= config.durabilityTick then
		return lib.notify({type = 'error', description = locale('petrolcan_not_enough_fuel')})
	end
	...
	CreateThread(function()
		lib.progressCircle({
			...
			canCancel = true,
			disable = {move = true, car = true, combat = true},
			anim = {dict = isPump and 'timetable@gardener@filling_can' or 'weapon@w_sp_jerrycan', clip = isPump and 'gar_ig_5_filling_can' or 'fire',},
		})
	...
	while state.isFueling do
		if isPump then
			...
			if price + config.priceTick >= moneyAmount then lib.cancelProgress() end
		...
		if GetVehiclePedIsIn(PlayerPedId(), false) ~= 0 then
			...
			if DoesEntityExist(vehicle) and GetPedInVehicleSeat(vehicle, -1) == PlayerPedId() then
				...
				elseif IsVehicleEngineOn(vehicle) then
					fuel.SetFuel(vehicle, currFuel - config.FuelUsage[Round(GetVehicleCurrentRpm(vehicle), 1)] * (config.FuelClasses[GetVehicleClass(vehicle)] or 1.0) / 10) 
				end
			end
		end
    end
