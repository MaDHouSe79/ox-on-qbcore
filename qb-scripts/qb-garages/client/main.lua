...
local function CanParkVehicle(veh, garageName, vehLocation)
    local garage = garageName and Garages[garageName] or
                       (CurrentGarage and Garages[CurrentGarage] or HouseGarages[CurrentHouseGarage])
    if not garage then
        return
    end
...
local function ParkVehicle(veh, garageName, vehLocation)
    ...
    QBCore.Functions.TriggerCallback('qb-garages:server:checkOwnership', function(owned)
        if owned then
            ...
            local totalFuel = exports[FuelScript]:GetFuel(veh)
            if not CanParkVehicle(veh, garageName, vehLocation) then
                return
            end
            TriggerServerEvent('qb-garages:server:updateVehicle', 1, totalFuel, engineDamage, bodyDamage, plate, garage)
    ...
    QBCore.Functions.TriggerCallback("qb-garages:server:GetGarageVehicles", function(result)
        if result == nil then
            ...
            for k, v in pairs(result) do
                ...
                local vname = QBCore.Shared.Vehicles[v.vehicle:lower()].name
                if v.state == 0 then
                ...
                elseif type ~= Lang:t('status.parked') and type ~= Lang:t("status.out") then
            ...
            if AllowSpawningFromAnywhere then
                ...
                for _, parkingSpot in ipairs(parkingSpots) do
                    local veh, distance = QBCore.Functions.GetClosestVehicle(
                        vector3(parkingSpot.x, parkingSpot.y, parkingSpot.z))
